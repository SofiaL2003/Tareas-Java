...
public class ConcurrentScraper {
    ...
    ConcurrentScraper(int threadCount) {
    ...
    List<PageResult> fetchAll(List<String> urls) {
    ...
    List<PageResult> fetchWithTimeout(List<String> urls, long timeoutMs) {
        List<CompletableFuture<PageResult>> futures = urls.stream()
            .map(url -> CompletableFuture.supplyAsync(
                    () -> fetchPage(url), executor)
    ...
    void printReport(List<PageResult> results) {
